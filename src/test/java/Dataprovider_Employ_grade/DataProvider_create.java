package Dataprovider_Employ_grade;
...
public class DataProvider_create {
	...
	@DataProvider(name="Invalid_gradename_test_blnk")
	public static Iterator<String[]> Invalid_designame_test_blnk() throws IOException
	{
		
		List<String[]> Obj = flagRowCount("Invalid_gradename_test_blnk");
		return Obj.iterator();	
	}
	
	@DataProvider(name="Invalid_gradecode_test_blnk")
	public static Iterator<String[]> Invalid_desigcode_test_blnk() throws IOException
	{
		
		List<String[]> Obj = flagRowCount("Invalid_gradecode_test_blnk");
		return Obj.iterator();	
	}

	@DataProvider(name="Invalid_gradename_test")
	public static Iterator<String[]> Invalid_designame_test() throws IOException
	{
		
		List<String[]> Obj = flagRowCount("Invalid_gradename_test");
		return Obj.iterator();	
	}
	
	@DataProvider(name="Invalid_gradecode_test")
	public static Iterator<String[]> Invalid_desigcode_test() throws IOException
	{
		
		List<String[]> Obj = flagRowCount("Invalid_gradecode_test");
		return Obj.iterator();	
	}
	
	@DataProvider(name="Valid_grade_test")
	public static Iterator<String[]> Valid_region_test() throws IOException
	{
		
		List<String[]> Obj = flagRowCount("Valid_grade_test");
		return Obj.iterator();	
	}
	...
	{
	//"D:\\akash\\salenium\\Reach_OAMC\\Excel-files\\Employee\\03_Reachwell_CP_Employ_grade_Data.xls"	
		ExcelReadWrite x=new ExcelReadWrite(System.getProperty("user.dir") + "\\Excel_sheets\\Employee\\03_Reachwell_CP_Employ_grade_Data.xls");
		HSSFSheet create_criti = x.Setsheet("create_employ_grade");
		...
		for(int xlRow=1;xlRow<=xlRowcount;xlRow++)
		{
			...
			if((Execute_Flag.equals("Y")&&(Scriptname.equals(script_name))))
				...
				arr_login[2]=x.Readvalue(create_criti, xlRow, "grade_name");
				arr_login[3]=x.Readvalue(create_criti, xlRow, "grade_code");
